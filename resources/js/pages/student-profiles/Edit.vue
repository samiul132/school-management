<template>
  <AppLayout>
    ...
    <div v-if="loading" class="text-center py-8">
      <div class="inline-flex items-center gap-2 text-lg text-gray-600">
                    ...
                    <div class="mt-1 flex items-center gap-4">
                      <div class="relative">
                        ...
                        >
                          ...
                          {{ form.student_image ? 'Change Photo' : 'Upload Photo' }}
                        </button>
                      </div>
                      <div v-if="form.student_image || studentImagePreview || form.student_image_url" class="flex items-center gap-2">
                        <div class="w-16 h-16 rounded-full overflow-hidden border-2 border-gray-200">
                          ...
                          />
                          <img 
                            v-else-if="form.student_image_url"
                            :src="form.student_image_url" 
                            alt="Student"
                            class="w-full h-full object-cover"
                          />
                        ...
                        <button
                          v-if="form.student_image || studentImagePreview || form.student_image_url"
                  ...
                  <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <v-select
                          v-model="presentDistrictObject"
                          :options="districts"
                          :filterable="true"
                          :searchable="true"
                          label="display_name"
                          :reduce="district => district"
                  ...
                  <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <v-select
                          v-model="permanentDistrictObject"
                          :options="districts"
                          :filterable="true"
                          :searchable="true"
                          label="display_name"
                  ...
                  <div>
                    ...
                    <div class="mt-1 flex items-center gap-4">
                      <div class="relative">
                        ...
                        >
                          <i class="fas fa-upload"></i>
                          {{ form.tc_image ? 'Change TC Certificate' : 'Upload TC Certificate' }}
                        </button>
                      </div>
                      <div v-if="form.tc_image || tcImagePreview || form.tc_image_url" class="flex items-center gap-2">
                        <div class="w-16 h-16 bg-gray-100 rounded-lg border border-gray-200 overflow-hidden">
                          ...
                          />
                          <img 
                            v-else-if="form.tc_image_url"
                            :src="form.tc_image_url" 
                            alt="TC"
                            class="w-full h-full object-cover"
                          />
                        ...
                        <button
                          v-if="form.tc_image || tcImagePreview || form.tc_image_url"
                          @click="removeTcImage"
                          class="text-red-500 hover:text-red-700"
                          type="button"
                        >
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <div class="space-y-2">
                ...
                <div v-if="form.gender" class="flex items-center justify-between">
                  <span class="text-sm text-gray-600">Gender</span>
                  <span class="text-sm font-medium text-gray-800">{{ form.gender.charAt(0).toUpperCase() + form.gender.slice(1) }}</span>
                </div>
                <div v-if="form.blood_group" class="flex items-center justify-between">
                  <span class="text-sm text-gray-600">Blood Group</span>
                  <span class="text-sm font-medium text-gray-800">{{ form.blood_group }}</span>
                </div>
                <div v-if="!form.date_of_birth && !form.gender && !form.blood_group" class="text-center text-gray-500 text-sm py-2">
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <ul class="text-sm text-blue-700 space-y-1">
                ...
                <li>• To remove image, click the X icon</li>
                <li>• Academic info can be updated later</li>
                <li>• Click delete to remove student</li>
              </ul>
            </div>
          </div>
        </div>
...
// Computed properties for v-select objects
const presentDistrictObject = computed({
  get() {
    if (!form.present_district_id) return null
    return districts.value.find(d => d.id === parseInt(form.present_district_id)) || null
  },
  set(value) {
    form.present_district_id = value ? value.id : ''
  }
})

const presentUpazilaObject = computed({
  get() {
    if (!form.present_upazila_id) return null
    return presentUpazilas.value.find(u => u.id === parseInt(form.present_upazila_id)) || null
  },
  set(value) {
    form.present_upazila_id = value ? value.id : ''
  }
})

const permanentDistrictObject = computed({
  get() {
    if (!form.permanent_district_id) return null
    return districts.value.find(d => d.id === parseInt(form.permanent_district_id)) || null
  },
  set(value) {
    form.permanent_district_id = value ? value.id : ''
  }
})

const permanentUpazilaObject = computed({
  get() {
    if (!form.permanent_upazila_id) return null
    return permanentUpazilas.value.find(u => u.id === parseInt(form.permanent_upazila_id)) || null
  },
  set(value) {
    form.permanent_upazila_id = value ? value.id : ''
  }
})
...
const copyPresentToPermanent = () => {
  if (sameAsPresentAddress.value) {
    ...
    // Present district to permanent upazilas set
...
// Watch for present district changes
watch(() => form.present_district_id, async (newDistrictId, oldDistrictId) => {
  // Clear upazila when district changes
  form.present_upazila_id = ''
  
  if (newDistrictId) {
    await fetchUpazilasByDistrict(newDistrictId, 'present')
  } else {
...
// Watch for permanent district changes
watch(() => form.permanent_district_id, async (newDistrictId, oldDistrictId) => {
  if (!sameAsPresentAddress.value) {
    form.permanent_upazila_id = ''
    
    if (newDistrictId) {
      await fetchUpazilasByDistrict(newDistrictId, 'permanent')
    } else {
      permanentUpazilas.value = []
    }
  }
...
const removeStudentImage = () => {
  form.student_image = 'REMOVE'
  studentImagePreview.value = null
  form.student_image_url = null
}
...
const removeTcImage = () => {
  form.tc_image = 'REMOVE'
  tcImagePreview.value = null
  form.tc_image_url = null
}
  ...
  try {
    ...
    if (response.data.success && response.data.data) {
      const upazilas = response.data.data.map(upazila => ({
        id: parseInt(upazila.id),
        name: upazila.name,
        bn_name: upazila.bn_name,
        district_id: parseInt(upazila.district_id),
...
const fetchStudentData = async () => {
  try {
    ...
    const response = await axios.get(`/api/student-profiles/${studentId}`)
    ...
    if (response.data.success && response.data.data) {
      ...
      Object.keys(form).forEach(key => {
        if (key !== 'academic_data' && key !== 'student_image_url' && key !== 'tc_image_url') {
          if (student[key] !== null && student[key] !== undefined) {
            if (key.includes('upazila_id') || key.includes('district_id')) {
              form[key] = parseInt(student[key]) || ''
            } else {
      ...
      // Blood group, religion, nationality, birth certificate number
      if (student.blood_group !== null && student.blood_group !== undefined) {
        form.blood_group = student.blood_group
        
        // Blood group options
        if (!bloodGroupOptions.value.includes(student.blood_group)) {
          bloodGroupOptions.value.push(student.blood_group)
        }
      }
      
      if (student.religion !== null && student.religion !== undefined) {
        form.religion = student.religion
        
        // Religion options
        if (!religionOptions.value.includes(student.religion)) {
          religionOptions.value.push(student.religion)
        }
      }
      ...
      if (student.nationality !== null && student.nationality !== undefined) {
        ...
        // Nationality options
        if (!nationalityOptions.value.includes(student.nationality)) {
          nationalityOptions.value.push(student.nationality)
        }
      }
      
      if (student.birth_certificate_number !== null && student.birth_certificate_number !== undefined) {
        form.birth_certificate_number = student.birth_certificate_number
      }
      
      // Set image URLs if available
      if (student.student_image_url) {
        form.student_image_url = student.student_image_url
      }
      if (student.tc_image_url) {
        form.tc_image_url = student.tc_image_url
      }
      
      // Populate academic data from class_wise_data
      if (student.class_wise_data && student.class_wise_data.length > 0) {
        const academicData = student.class_wise_data[0]
        form.academic_data = {
          class_id: academicData.class_id || null,
          shift_id: academicData.shift_id || null,
          version_id: academicData.version_id || null,
          session_id: academicData.session_id || null,
          section_id: academicData.section_id || null,
          class_roll: academicData.class_roll || null
        }
      }
      
      // Check if addresses are same
      if (student.present_village && student.permanent_village &&
          student.present_village === student.permanent_village &&
          student.present_post_office === student.permanent_post_office &&
          student.present_upazila_id === student.permanent_upazila_id &&
          student.present_district_id === student.permanent_district_id) {
        sameAsPresentAddress.value = true
      }
      
      // Wait for districts to load first
      await fetchDistricts()
      
      // Fetch upazilas for present address if district exists
      if (student.present_district_id) {
        await fetchUpazilasByDistrict(student.present_district_id, 'present')
        
        // Small delay to ensure upazilas are loaded
        setTimeout(() => {
          if (student.present_upazila_id) {
            form.present_upazila_id = parseInt(student.present_upazila_id)
          }
        }, 300)
      }
      
      // Fetch upazilas for permanent address if district exists
      if (student.permanent_district_id && !sameAsPresentAddress.value) {
        await fetchUpazilasByDistrict(student.permanent_district_id, 'permanent')
        
        // Small delay to ensure upazilas are loaded
        setTimeout(() => {
          if (student.permanent_upazila_id) {
            form.permanent_upazila_id = parseInt(student.permanent_upazila_id)
          }
        }, 300)
      }
      
      // If same address, copy present to permanent
      if (sameAsPresentAddress.value && student.present_district_id) {
        form.permanent_village = form.present_village
        form.permanent_post_office = form.present_post_office
        form.permanent_district_id = form.present_district_id
    ...
    showErrorAlert('Error', 'Failed to load student data. Please try again.')
...
const submitForm = async () => {
  ...
  try {
    showLoadingAlert('Updating student admission...')
    ...
    Object.keys(form).forEach(key => {
      ...
      } else if (key === 'student_image') {
        if (form[key] === 'REMOVE') {
          formData.append('remove_student_image', '1') 
        } else if (form[key] instanceof File) {
          formData.append('student_image', form[key])
        }
      } else if (key === 'tc_image') {
        if (form[key] === 'REMOVE') {
          formData.append('remove_tc_image', '1') 
        } else if (form[key] instanceof File) {
          formData.append('tc_image', form[key])
        }
      } else if (key !== 'student_image_url' && key !== 'tc_image_url') {
        // Append other fields including blood_group, religion, nationality
    ...
    const response = await axios.post(`/api/student-profiles/${studentId}?_method=PUT`, formData, {
      headers: {
    ...
    if (error.response?.status === 422) {
      ...
      if (errors.value.remove_student_image) {
        errorMessage = errors.value.remove_student_image[0]
      } else if (errors.value.remove_tc_image) {
        errorMessage = errors.value.remove_tc_image[0]
      }
      
      showErrorAlert('Validation Error', errorMessage)
    } else {
      console.error('Error updating student admission:', error)
      showErrorAlert('Error', 'Failed to update student admission. Please try again.')
    }
  }
}
...
const addCustomBloodGroup = (newTag) => {
  if (!bloodGroupOptions.value.includes(newTag)) {
    bloodGroupOptions.value.push(newTag)
  }
  form.blood_group = newTag
}

const addCustomReligion = (newTag) => {
  if (!religionOptions.value.includes(newTag)) {
    religionOptions.value.push(newTag)
  }
  form.religion = newTag
}

const addCustomNationality = (newTag) => {
  if (!nationalityOptions.value.includes(newTag)) {
    nationalityOptions.value.push(newTag)
  }
