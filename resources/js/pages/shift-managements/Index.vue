<template>
  <AppLayout>
      ...
      <div class="flex flex-col md:flex-row justify-between md:items-center gap-4">
        ...
        <div>
          ...
          <p class="text-gray-600 mt-2">
            Manage your academic shifts and timings
          </p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      <div class="bg-white rounded-2xl shadow-lg p-3 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 text-sm font-medium">Total Shifts</p>
            <p class="text-3xl font-bold text-gray-800">{{ totalShifts }}</p>
          </div>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-3 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 text-sm font-medium">Active Shifts</p>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-3 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 text-sm font-medium">Inactive Shifts</p>
            <p class="text-3xl font-bold text-gray-600">{{ inactiveShifts }}</p>
          </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <table class="w-full min-w-[800px]">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-40">Start Time</th>
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="shift in filteredShifts" :key="shift.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-800">
                <span class="inline-flex items-center justify-center px-3 py-1 bg-blue-100 text-blue-800 rounded-full font-semibold">
              ...
              <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">
                <div class="relative">
                  ...
                  <button 
                    ...
                    :class="{
                      'bg-blue-600 text-white': openDropdownId !== shift.id,
                      'bg-gray-200 text-black': openDropdownId === shift.id
                    }"
                  >
                    Actions
                    <i 
                      ...
                      :class="{
                        'rotate-180': openDropdownId === shift.id,
                        'text-white': openDropdownId !== shift.id,
                        'text-black': openDropdownId === shift.id
                      }"
                    ></i>
                  </button>
              ...
              <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-800">
                <div class="flex items-center gap-2">
                  ...
                  <div class="truncate max-w-[200px]" :title="shift.shift_name">
                    {{ shift.shift_name }}
                  </div>
                </div>
              </td>
              ...
              <!-- Start Time -->
              <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-800">
                <span class="inline-flex items-center px-3 py-1.5 bg-gray-100 text-gray-800 rounded-lg font-medium">
              ...
              <td class="px-4 py-4 whitespace-nowrap">
                <span 
                  ...
                  :class="{
                    ...
                    'bg-gray-100 text-gray-800': shift.status === 'inactive'
                  }"
                >
                  <i 
                    class="fas fa-circle mr-1.5 text-[10px]"
                    :class="{
                      'text-green-500': shift.status === 'active',
                      'text-gray-500': shift.status === 'inactive'
                    }"
                  ></i>
              ...
              <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-800">
                <div class="flex flex-col">
                  ...
                  <span class="text-xs text-gray-500">{{ formatTime(shift.created_at) }}</span>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <!-- Empty State -->
      <div v-if="!loading && shifts.length === 0" class="p-8 text-center">
      ...
      <!-- No Search Results -->
      <div v-if="!loading && shifts.length > 0 && filteredShifts.length === 0" class="p-8 text-center">
        <i class="fas fa-search text-4xl text-gray-400 mb-4"></i>
        <p class="text-gray-600">No shifts match your search criteria.</p>
        <button 
          @click="clearFilters"
    ...
    >
      ...
      >
        ...
        <div class="p-6 max-h-[70vh] overflow-y-auto">
          <form @submit.prevent="isEditing ? updateShift() : createShift()">
            <!-- Shift Name -->
            <div class="mb-6">
              ...
              <input 
                ...
                :disabled="submitting"
              >
              <p v-if="formErrors.shift_name" class="mt-2 text-sm text-red-600">
            ...
            <div class="mb-6">
              ...
              <input 
                ...
                :disabled="submitting"
              >
              <p v-if="formErrors.start_time" class="mt-2 text-sm text-red-600">
            ...
            <div class="mb-6">
              ...
              <input 
                ...
                :disabled="submitting"
              >
              <p v-if="formErrors.end_time" class="mt-2 text-sm text-red-600">
                {{ formErrors.end_time }}
              </p>
              <p v-if="formErrors.end_time_validation" class="mt-2 text-sm text-red-600">
                {{ formErrors.end_time_validation }}
              </p>
            </div>
...
const fetchShifts = async () => {
  try {
    loading.value = true
    const response = await axios.get('/api/shift-managements')
    ...
    showErrorAlert('Error', 'Failed to load shifts')
  } finally {
    loading.value = false
  }
}
...
const createShift = async () => {
  try {
    ...
    if (form.value.end_time <= form.value.start_time) {
      formErrors.value = {
        end_time_validation: 'End time must be after start time'
      }
      submitting.value = false
      return
    }
    
    showLoadingAlert('Creating shift...')

    const response = await axios.post('/api/shift-managements', form.value)
    ...
    if (response.status === 201) {
      ...
      await showSuccessAlert('Success!', 'Shift created successfully.')
    }
    ...
    if (error.response?.status === 422) {
      ...
      if (error.response.data.error === 'End time must be after start time') {
        formErrors.value.end_time_validation = error.response.data.error
      }
      showErrorAlert('Validation Error', 'Please check the form for errors.')
    } else {
      showErrorAlert('Error', 'Failed to create shift')
    }
  } finally {
    submitting.value = false
  }
...
const updateShift = async () => {
  try {
    ...
    showLoadingAlert('Updating shift...')

    const response = await axios.put(`/api/shift-managements/${editingShiftId.value}`, form.value)
    
    closeAlert()
    
    if (response.status === 200) {
      const index = shifts.value.findIndex(s => s.id === editingShiftId.value)
      if (index !== -1) {
      ...
      await showSuccessAlert('Success!', 'Shift updated successfully.')
    }
    ...
    if (error.response?.status === 422) {
      ...
      showErrorAlert('Error', 'Shift not found')
      showModal.value = false
      form.value = {
        shift_name: '',
        start_time: '09:00',
      ...
      showErrorAlert('Error', 'Failed to update shift')
    }
  } finally {
    submitting.value = false
  }
...
const deleteShift = async (id) => {
  ...
  if (result.isConfirmed) {
    try {
      showLoadingAlert('Deleting shift...')
      
      await axios.delete(`/api/shift-managements/${id}`)
      
      shifts.value = shifts.value.filter(shift => shift.id !== id)
      
      closeAlert()
      await showSuccessAlert('Deleted!', 'Shift deleted successfully.')
      
    } catch (error) {
      closeAlert()
      console.error('Error deleting shift:', error)
      showErrorAlert('Error', 'Failed to delete shift')
    }
  }
}
...
const activeShifts = computed(() => 
  shifts.value.filter(shift => shift.status === 'active').length
)

const inactiveShifts = computed(() => 
  shifts.value.filter(shift => shift.status === 'inactive').length
)
...
const filteredShifts = computed(() => {
  ...
  if (statusFilter.value) {
    filtered = filtered.filter(shift => shift.status === statusFilter.value)
  }
