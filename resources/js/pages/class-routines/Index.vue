  ...
  <AppLayout>
    ...
    <div class="mt-2 mb-1">
      <div class="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          <h1 class="text-3xl font-bold text-gray-800">Class Routines</h1>
        </div>
        <router-link 
          to="/class-routines/create"
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 mb-3">
      <div class="px-3 py-2">
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div>
            ...
            >
              ...
              <template #selected-option="{ session_name, status }">
                <div class="flex items-center gap-2">
                  <span>{{ session_name }}</span>
                </div>
              </template>
        ...
        <div class="flex justify-end mt-2">
            <button 
                @click="applyFilters"
                class="w-20 mr-2 px-2 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors cursor-pointer"
            >
                Search
            </button>
            <button 
                @click="resetFilters"
                class="w-20 px-2 py-1.5 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-lg font-medium transition-colors cursor-pointer"
            >
                Clear
            </button>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100">
      <div class="px-4 py-2 border-b border-gray-200 flex justify-between items-center">
        <h2 class="text-base font-semibold text-gray-800">Class Routines List</h2>
      ...
      <div v-if="loading" class="p-12 text-center">
        ...
        <p class="mt-2 text-gray-600">Loading class routines...</p>
      </div>

      <!-- Empty State -->
      <div v-else-if="classRoutines.data.length === 0" class="p-12 text-center">
        <i class="fas fa-calendar-alt text-4xl text-gray-400 mb-3"></i>
        <h3 class="text-lg font-medium text-gray-700">No class routines found</h3>
        <p class="text-gray-600 mt-1">Get started by creating a new class routine</p>
        <router-link 
          to="/class-routines/create"
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Section</th>
              <th class="px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Version</th>
              <th class="px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Shift</th>
              <th class="px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Periods</th>
              <th class="px-3 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Created</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200">
            <tr v-for="(item, index) in classRoutines.data" :key="item.id" class="hover:bg-gray-50">
              <!-- SL -->
              <td class="px-3 py-2 whitespace-nowrap">
                <div class="text-sm font-medium text-gray-900">
                  {{ (classRoutines.current_page - 1) * perPage + index + 1 }}
                </div>
              </td>
              ...
              <!-- Class -->
              <td class="px-3 py-2 whitespace-nowrap">
                <span class="inline-flex items-center gap-1.5 px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">
              ...
              <!-- Shift -->
              <td class="px-3 py-2 whitespace-nowrap">
                <span class="inline-flex items-center gap-1.5 px-2 py-1 text-xs font-medium rounded-full bg-orange-100 text-orange-800">
              ...
              <td class="px-3 py-2 whitespace-nowrap">
                <div class="flex items-center gap-1.5">
                  <span class="text-base font-bold text-green-600">{{ item.number_of_periods }}</span>
                  <span class="text-xs text-gray-500">periods</span>
                </div>
              </td>
        ...
        <!-- Pagination -->
        <div v-if="classRoutines.data.length > 0" class="px-4 py-3 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
          <div class="text-sm text-gray-700">
            Showing {{ classRoutines.from }} to {{ classRoutines.to }} of {{ classRoutines.total }} entries
          </div>
          
          <div class="flex items-center gap-2">
            <button 
              @click="changePage(classRoutines.current_page - 1)"
              :disabled="!classRoutines.prev_page_url"
            ...
            <div class="flex items-center gap-1">
              <button 
                ...
                :class="[
                  ...
                  page === classRoutines.current_page 
            ...
            <button 
              @click="changePage(classRoutines.current_page + 1)"
              :disabled="!classRoutines.next_page_url"
              class="px-3 py-1.5 border border-gray-300 rounded text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50"
            >
              Next
            </button>
  ...
  try {
    ...
    if (response.data.success) {
      ...
      if (response.data.data.active_session) {
        filters.value.session_id = response.data.data.active_session.id
      }
    }
  } catch (error) {
...
const loadClassRoutines = async (page = 1) => {
  loading.value = true
  ...
  try {
    ...
    const response = await axios.get('/api/class-routines', { params })
    
    if (response.data.success) {
      classRoutines.value = response.data.data
    } else {
      showErrorAlert('Error', response.data.message)
    }
  } catch (error) {
    console.error('Failed to load class routines:', error)
    showErrorAlert('Error', 'Failed to load class routines')
  } finally {
    loading.value = false
  }
}
...
const changePage = (page) => {
  if (page >= 1 && page <= classRoutines.value.last_page) {
    loadClassRoutines(page)
  }
}

const getPaginationPages = () => {
  const current = classRoutines.value.current_page
  const last = classRoutines.value.last_page
  const delta = 2
  const range = []
  const rangeWithDots = []
  let l
...
const handleEdit = (item) => {
  router.push(`/class-routines/${item.id}/edit`)
  closeDropdown()
}
...
const handleDelete = async (item) => {
  const result = await showConfirmDialog(
    ...
    `Are you sure you want to delete routine for ${item.class?.class_name} - ${item.section?.section_name}?`
  )
...
const deleteClassRoutine = async (id) => {
  try {
    const response = await axios.delete(`/api/class-routines/${id}`)
    ...
    showErrorAlert('Error', error.response?.data?.message || 'Failed to delete class routine')
  }
}
...
onMounted(async () => {
  ...
  // Load routines after dropdown data is loaded (with active session selected)
  await loadClassRoutines()
})
</script>
