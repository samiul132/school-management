<template>
  <AppLayout>
    ...
    <div class="mb-1 mt-2 flex flex-col md:flex-row justify-between md:items-center gap-2">
      <div>
        <h1 class="text-xl font-bold text-gray-800">Create Class Routine</h1>
      </div>
      <router-link 
        to="/class-routines"
    ...
    <div class="bg-white rounded-xl shadow border border-gray-100 p-3">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pt-3 border-t border-gray-200">
        <div class="text-xs text-gray-600">
          ...
          Fill in all periods for each day to create the routine
        </div>
...
// Form - Start with null, will show 1 period by default in display
const form = reactive({
  session_id: null,
  version_id: null,
  shift_id: null,
...
// Computed value for active days (excluding off days)
const activeDays = computed(() => {
  if (!form.off_days || form.off_days.length === 0) {
    return allDays
  }
  return allDays.filter(day => !form.off_days.includes(day))
})
  ...
  try {
    ...
    if (response.data.success) {
      ...
      if (data.active_session) {
        form.session_id = data.active_session.id
      }
    }
  } catch (error) {
...
const generatePeriodColumns = () => {
  const periods = form.number_of_periods
  
  if (periods < 1) {
    form.number_of_periods = 1
    return
  }
...
const getOffDayLabels = () => {
  return form.off_days.join(', ')
}
...
const saveRoutine = async () => {
  ...
  // Prepare routine details (only for active days)
  ...
  showLoadingAlert('Saving class routine...')
  ...
  try {
    const payload = {
      ...
      off_day: form.off_days.join(','), // Convert array to comma-separated string
      routine_details: routine_details
    }

    const response = await axios.post('/api/class-routines', payload)

    closeAlert()

    if (response.data.success) {
      await showSuccessAlert('Success!', 'Class routine created successfully')
    ...
    if (error.response?.data?.message) {
      ...
      showErrorAlert('Error', 'Failed to save routine. Please try again.')
    }
  } finally {
...
// Lifecycle
onMounted(() => {
  // Initialize period times for default period
  periodTimes.value[1] = ''
