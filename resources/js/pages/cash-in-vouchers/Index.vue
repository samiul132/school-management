<template>
  <AppLayout>
    ...
    <div>
        ...
        Manage all incoming cash transactions
        </p>
    </div>

    <!-- Button -->
    <router-link 
        :to="{ name: 'cash-in-vouchers.create' }"
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-4">
      <div class="bg-white rounded-2xl shadow-lg p-3 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 text-sm font-medium">Total Cash In</p>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-3 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-green-600">{{ thisMonthVouchers }}</p>
          </div>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-3 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 text-sm font-medium">Today's Cash In</p>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
          ...
          <div class="relative">
            <input 
              type="text" 
              v-model="search"
              placeholder="Search cash in vouchers..." 
              class="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 bg-white text-gray-800 w-full sm:w-64"
            >
          ...
          <select 
            ...
            class="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 bg-white text-gray-800"
          >
        ...
        <table class="w-full min-w-[1200px]">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="voucher in paginatedVouchers" :key="voucher.id" class="hover:bg-gray-50">
                ...
                <div class="relative">
                  ...
                  >
                    <div class="py-1">
                      <!-- View Option -->
                      <router-link 
                        :to="{ name: 'cash-in-vouchers.show', params: { id: voucher.id } }"
                      ...
                      <!-- Edit Option -->
                      <router-link 
                        :to="{ name: 'cash-in-vouchers.edit', params: { id: voucher.id } }"
                        class="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition-colors"
                        @click="closeDropdown"
                      >
              ...
              <!-- Amount -->
              <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-800 font-mono font-semibold text-green-600">
      ...
      <div v-if="loading" class="p-8 text-center">
        ...
        <p class="mt-2 text-gray-600">Loading cash in vouchers...</p>
      </div>

      <!-- Empty State -->
      <div v-if="!loading && vouchers.length === 0" class="p-8 text-center">
        <i class="fas fa-download text-4xl text-gray-400 mb-4"></i>
        <p class="text-gray-600">No cash in vouchers found.</p>
        <router-link 
          :to="{ name: 'cash-in-vouchers.create' }"
          class="inline-block mt-4 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg font-semibold transition-colors"
        >
          Record First Cash In
        </router-link>
      </div>
      ...
      <div v-if="!loading && vouchers.length > 0 && filteredVouchers.length === 0" class="p-8 text-center">
        ...
        <p class="text-gray-600">No cash in vouchers match your search criteria.</p>
        <button 
          @click="clearFilters"
      ...
      <div v-if="!loading && filteredVouchers.length > 0" class="px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div class="text-sm text-gray-700">
          Showing {{ showingStart }} to {{ showingEnd }} of {{ filteredVouchers.length }} cash in transactions
        </div>
...
const cashInVouchers = computed(() => 
  vouchers.value.filter(voucher => voucher.voucher_type === 'CREDIT')
)

const totalVouchers = computed(() => cashInVouchers.value.length)

const todayVouchers = computed(() => {
  const today = new Date().toISOString().split('T')[0] 
  
  return cashInVouchers.value.filter(voucher => {
...
const totalAmount = computed(() => 
  cashInVouchers.value.reduce((sum, voucher) => sum + parseFloat(voucher.amount), 0)
)

const uniqueAccounts = computed(() => {
  const accounts = cashInVouchers.value
...
const filteredVouchers = computed(() => {
  let filtered = cashInVouchers.value
...
const fetchVouchers = async () => {
  try {
    loading.value = true
    // console.log('Fetching all vouchers for cash in...')
    const response = await axios.get('/api/vouchers')
    // console.log('Vouchers API response:', response.data)
    
    // Filter only CREDIT vouchers for cash in
    const cashInVouchers = response.data.filter(voucher => voucher.voucher_type === 'CREDIT')
    // console.log('Cash In vouchers:', cashInVouchers)
    
    vouchers.value = cashInVouchers
    
    // Log first voucher to check relationships
    // if (cashInVouchers.length > 0) {
    //   console.log('First cash in voucher data:', cashInVouchers[0])
    //   console.log('Account data:', cashInVouchers[0].account)
    //   console.log('Subsidiary data:', cashInVouchers[0].subsidiary)
    //   console.log('Head data:', cashInVouchers[0].head)
    // }
    
  } catch (error) {
    console.error('Error fetching vouchers:', error)
    console.error('Error details:', error.response)
    showErrorAlert('Error', 'Failed to load cash in vouchers')
  } finally {
    loading.value = false
  }
}
...
const deleteVoucher = async (id) => {
  const result = await showConfirmDialog(
    'Are you sure?',
    'This cash in transaction will be permanently deleted!'
  )

  if (result.isConfirmed) {
    try {
      showLoadingAlert('Deleting cash in voucher...')
      
      await axios.delete(`/api/vouchers/${id}`)
      vouchers.value = vouchers.value.filter(voucher => voucher.id !== id)
      
      closeAlert()
      await showSuccessAlert('Deleted!', 'Cash in voucher has been deleted.')
      
    } catch (error) {
      closeAlert()
      console.error('Error deleting cash in voucher:', error)
      showErrorAlert('Error', 'Failed to delete cash in voucher')
    }
  }
}
...
onMounted(() => {
  ...
  if (route.query.created === 'true') {
    ...
    router.replace({ name: 'cash-in-vouchers.index' })
  }
  
  if (route.query.updated === 'true') {
    showSuccessAlert('Success!', 'Cash in voucher updated successfully.')
    router.replace({ name: 'cash-in-vouchers.index' })
  }
})
