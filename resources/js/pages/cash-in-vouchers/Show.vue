<template>
  <AppLayout>
      ...
      <div class="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          <h1 class="text-3xl font-bold text-gray-800">
            Cash In Voucher Details
          </h1>
          <p class="text-gray-600">
            View incoming cash transaction information
          </p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
          ...
          <!-- Edit Button -->
          <router-link
            :to="{ name: 'cash-in-vouchers.edit', params: { id: $route.params.id } }"
            class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-green-600 hover:bg-green-700 
    ...
    <div v-if="loading" class="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 text-center">
      ...
      <p class="text-gray-800 font-semibold">Loading cash in voucher details...</p>
    </div>
    ...
    <div v-else-if="error" class="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 text-center">
      ...
      <router-link 
        :to="{ name: 'cash-in-vouchers.index' }"
        class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg font-semibold transition-colors"
      >
        <i class="fas fa-arrow-left"></i>
    ...
    <!-- Not Cash In Voucher Error -->
    <div v-else-if="voucher && voucher.voucher_type !== 'CREDIT'" class="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 text-center">
      <i class="fas fa-exclamation-circle text-3xl text-orange-600 mb-4"></i>
      <p class="text-gray-800 font-semibold mb-2">This is not a Cash In Voucher</p>
      <p class="text-gray-600 mb-4">This voucher is a {{ voucher.voucher_type }} voucher, not a cash in transaction.</p>
      <router-link 
        :to="{ name: 'cash-in-vouchers.index' }"
        class="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg font-semibold transition-colors"
      >
      ...
      <div class="lg:col-span-2">
        ...
        <div v-if="voucher.module_name || voucher.ref_module_id" class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mb-6">
          <div class="px-6 py-4 border-b border-gray-200 bg-blue-50">
            <h3 class="text-lg font-semibold text-gray-800">
              <i class="fas fa-source text-blue-600 mr-2"></i>
              Source Information
            </h3>
          </div>
          <div class="p-6">
          ...
          <div class="p-6">
            <div class="space-y-4">
              ...
              <div class="flex justify-between items-center p-3 bg-green-50 rounded-lg border border-green-200">
                <span class="text-sm font-medium text-green-700">Amount Received:</span>
              ...
              <!-- Type -->
              <div class="flex justify-between items-center p-3 bg-green-50 rounded-lg border border-green-200">
                <span class="text-sm font-medium text-green-700">Transaction Type:</span>
...
const fetchVoucher = async () => {
  try {
    ...
    console.log('Cash In Voucher API Response:', response.data)
    
    // Check if this is actually a cash in voucher (CREDIT)
    if (response.data.voucher_type !== 'CREDIT') {
    ...
    console.error('Error fetching cash in voucher:', err)
    
    if (err.response?.status === 404) {
      error.value = 'Cash in voucher not found'
    } else if (err.response?.status === 500) {
      error.value = 'Server error occurred'
    } else {
      error.value = 'Failed to load cash in voucher details'
    }
  } finally {
    loading.value = false
  }
...
@media print {
  ...
  .bg-green-50, .bg-blue-50, .bg-purple-50, .bg-gray-50 {
