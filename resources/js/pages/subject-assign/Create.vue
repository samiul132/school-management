      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 pt-3 border-t border-gray-200">
        ...
        <div class="flex items-center gap-2">
          ...
          <button 
            ...
            class="px-4 py-2 text-sm bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-lg font-semibold transition-colors flex items-center gap-1 shadow"
          >
...
const fetchDropdownData = async () => {
  try {
    ...
    if (activeSession) {
      form.session_id = activeSession.id
    }
...
const saveAssignment = async () => {
  ...
  showLoadingAlert('Saving subject assignment...')
  ...
  try {
    ...
    const response = await axios.post('/api/subject-assigns', data)

    closeAlert()

    if (response.data.success) {
      showSuccessAlert('Success!', 'Subject assignment created successfully')
    ...
    if (error.response?.data?.message) {
      ...
      showErrorAlert('Error', 'Failed to save assignment. Please try again.')
    }
  } finally {
