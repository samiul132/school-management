      ...
      <div class="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          <h1 class="text-3xl font-bold text-gray-800">
            Edit Cash Out Voucher
          </h1>
          <p class="text-gray-600">
            Update outgoing cash transaction details
          </p>
        </div>
        <div class="flex items-center gap-3 mb-2">
          <router-link 
            :to="{ name: 'cash-out-vouchers.index' }"
            class="inline-flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg shadow transition-colors font-semibold"
          >
          ...
          <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-200">
            ...
            <div>
              ...
              <p class="text-sm text-gray-600">
                Update the details of outgoing cash transaction
              </p>
            </div>
          </div>
            ...
            <div class="space-y-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    id="date"
                    v-model="form.date"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 bg-white text-gray-800"
                ...
                <!-- Voucher Type (Auto DEBIT - Readonly) -->
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">
                    Voucher Type
                  </label>
                  <div class="w-full px-4 py-2 border border-red-300 rounded-lg bg-red-50 text-red-800 font-semibold">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    step="0.01"
                    min="0"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 bg-white text-gray-800 placeholder-gray-500"
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Office Supplies, Utilities, Salary, etc."
                  />
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="4"
                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 bg-white text-gray-800 placeholder-gray-500 resize-y"
                  :class="{ 'border-red-500': errors.description }"
                  placeholder="Enter description of the cash out transaction..."
                ></textarea>
          ...
          <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
            <div class="flex flex-col sm:flex-row justify-end gap-3">
              ...
              >
                ...
                {{ loading ? 'Updating...' : 'Update Cash Out' }}
              </button>
            </div>
          </div>
        </div>
      ...
      <div class="lg:col-span-1">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-6">
          <div class="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 class="text-xl font-semibold text-gray-800">Cash Out Summary</h3>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <div class="p-3 bg-red-50 rounded-lg border border-red-200">
                <div class="flex items-center gap-2">
                  <i class="fas fa-arrow-up text-red-600"></i>
                  <span class="text-red-800 font-semibold">CASH OUT (DEBIT)</span>
                </div>
              </div>
            </div>
            ...
            <div v-if="selectedAccount" class="space-y-2">
              ...
              <div class="p-3 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <p class="text-xs text-red-600 mt-1">
                  After update: {{ formatCurrency((selectedAccount.balance || 0) - (parseFloat(form.amount) || 0)) }}
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-6">
          ...
          <div class="space-y-3">
            <div v-if="selectedAccount" class="flex items-center justify-between p-2 bg-red-50 rounded border border-red-200">
              <span class="text-sm text-red-800 font-medium">Account</span>
              <span class="text-sm text-red-600">{{ selectedAccount.name }}</span>
            </div>
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <ul class="text-sm text-red-700 space-y-1">
                ...
                <li>• Voucher type is fixed as DEBIT</li>
                <li>• Select the source account</li>
                <li>• Account balance will be updated</li>
                <li>• Add expense description for tracking</li>
...
const form = reactive({
  date: '',
  voucher_type: 'DEBIT', // Always DEBIT for cash out
  account_id: null,
...
const fetchVoucherData = async () => {
  try {
    ...
    // Ensure voucher is DEBIT type (cash out)
    if (voucherData.voucher_type !== 'DEBIT') {
      showErrorAlert('Error', 'This is not a cash out voucher')
      router.push({ name: 'cash-out-vouchers.index' })
      return
    }
    
    Object.assign(form, {
      date: voucherData.date ? voucherData.date.split('T')[0] : '',
      voucher_type: 'DEBIT', // Force DEBIT type
      account_id: voucherData.account_id || null,
      subsidiaries_id: voucherData.subsidiaries_id || null,
      head_id: voucherData.head_id || null,
      amount: voucherData.amount || 0,
      description: voucherData.description || '',
      module_name: voucherData.module_name || '',
      ref_module_id: voucherData.ref_module_id || null
    })
    
  } catch (error) {
    console.error('Error fetching voucher data:', error)
    showErrorAlert('Error', 'Failed to load cash out voucher data')
    router.push({ name: 'cash-out-vouchers.index' })
  }
}
...
const validateForm = () => {
  ...
  if (selectedAccount.value && form.amount > 0) {
    const currentBalance = selectedAccount.value.balance || 0
    if (parseFloat(form.amount) > currentBalance) {
      newErrors.amount = ['Insufficient account balance']
    }
  }
...
const submitForm = async () => {
  ...
  try {
    showLoadingAlert('Updating cash out voucher...')
    ...
    const formData = {
      ...
      voucher_type: 'DEBIT' // Ensure it's always DEBIT
    }

    await axios.put(`/api/vouchers/${voucherId}`, formData)
    ...
    if (error.response?.status === 422) {
      ...
      showErrorAlert('Error', 'Cash out voucher not found')
      router.push({ name: 'cash-out-vouchers.index' })
    } else {
      console.error('Error updating cash out voucher:', error)
      showErrorAlert('Error', 'Failed to update cash out voucher. Please try again.')
    }
  }
}
