<template>
  <AppLayout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      <div class="bg-white rounded-2xl shadow-lg p-3 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 text-sm font-medium">Total Versions</p>
            <p class="text-3xl font-bold text-gray-800">{{ totalVersions }}</p>
          </div>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-3 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 text-sm font-medium">Active Versions</p>
            <p class="text-3xl font-bold text-green-600">{{ activeVersions }}</p>
          </div>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-3 border border-gray-100">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 text-sm font-medium">Inactive Versions</p>
            <p class="text-3xl font-bold text-gray-600">{{ inactiveVersions }}</p>
          </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <tr v-for="version in filteredVersions" :key="version.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">
                <div class="relative">
                  ...
                  <button 
                    ...
                    :class="{
                      'bg-blue-600 text-white': openDropdownId !== version.id,
                      'bg-gray-200 text-black': openDropdownId === version.id
                    }"
                  >
                    Actions
                    <i 
                      ...
                      :class="{
                        'rotate-180': openDropdownId === version.id,
                        'text-white': openDropdownId !== version.id,
                        'text-black': openDropdownId === version.id
                      }"
                    ></i>
                  </button>
                  ...
                  <!-- Dropdown Menu -->
                  <div 
                    v-if="openDropdownId === version.id"
              ...
              <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-gray-800">
                <div class="flex items-center gap-2">
                  ...
                  <div class="truncate max-w-[200px]" :title="version.version_name">
              ...
              <td class="px-4 py-4 whitespace-nowrap">
                <span 
                  ...
                  :class="{
                    'bg-green-100 text-green-800': version.status === 'active',
                ...
                >
                  <i 
                    ...
                    :class="{
                      'text-green-500': version.status === 'active',
                      'text-gray-500': version.status === 'inactive'
                    }"
                  ></i>
              ...
              <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-800">
                <div class="flex flex-col">
                  ...
                  <span class="text-xs text-gray-500">{{ formatTime(version.updated_at) }}</span>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <!-- Empty State -->
      <div v-if="!loading && versions.length === 0" class="p-8 text-center">
      ...
      <!-- No Search Results -->
      <div v-if="!loading && versions.length > 0 && filteredVersions.length === 0" class="p-8 text-center">
        <i class="fas fa-search text-4xl text-gray-400 mb-4"></i>
        <p class="text-gray-600">No versions match your search criteria.</p>
        <button 
          @click="clearFilters"
      ...
      >
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white rounded-t-2xl z-10">
          <h3 class="text-lg font-semibold text-gray-800">
            {{ isEditing ? 'Edit Version' : 'Create New Version' }}
          </h3>
          <button 
            @click="closeModal"
        ...
        <div class="p-6 max-h-[70vh] overflow-y-auto">
          <form @submit.prevent="isEditing ? updateVersion() : createVersion()">
            <!-- Version Name -->
            <div class="mb-6">
              ...
              <input 
                ...
                placeholder="Enter version name (e.g., English Version, Bangla Version)"
                required
                :disabled="submitting"
              >
              <p v-if="formErrors.version_name" class="mt-2 text-sm text-red-600">
...
const fetchVersions = async () => {
  try {
    loading.value = true
    const response = await axios.get('/api/version-managements')
    ...
    showErrorAlert('Error', 'Failed to load versions')
  } finally {
    loading.value = false
  }
}
...
const createVersion = async () => {
  try {
    ...
    showLoadingAlert('Creating version...')

    const response = await axios.post('/api/version-managements', form.value)
    ...
    if (response.status === 201) {
      ...
      await showSuccessAlert('Success!', 'Version created successfully.')
    }
    ...
    if (error.response?.status === 422) {
      ...
      showErrorAlert('Error', 'Failed to create version')
    }
  } finally {
...
const updateVersion = async () => {
  try {
    ...
    showLoadingAlert('Updating version...')

    const response = await axios.put(`/api/version-managements/${editingVersionId.value}`, form.value)
    
    closeAlert()
    
    if (response.status === 200) {
      const index = versions.value.findIndex(v => v.id === editingVersionId.value)
      if (index !== -1) {
        versions.value[index] = response.data.data
      }
      ...
      await showSuccessAlert('Success!', 'Version updated successfully.')
    }
    ...
    if (error.response?.status === 422) {
      ...
      showErrorAlert('Error', 'Version not found')
      showModal.value = false
      form.value = {
        version_name: '',
        status: 'active'
      ...
      showErrorAlert('Error', 'Failed to update version')
    }
  } finally {
    submitting.value = false
  }
...
const deleteVersion = async (id) => {
  ...
  if (result.isConfirmed) {
    try {
      showLoadingAlert('Deleting version...')
      
      await axios.delete(`/api/version-managements/${id}`)
      
      versions.value = versions.value.filter(version => version.id !== id)
      
      closeAlert()
      await showSuccessAlert('Deleted!', 'Version deleted successfully.')
      
    } catch (error) {
      closeAlert()
      console.error('Error deleting version:', error)
      showErrorAlert('Error', 'Failed to delete version')
    }
  }
}
...
const activeVersions = computed(() => 
  versions.value.filter(version => version.status === 'active').length
)

const inactiveVersions = computed(() => 
  versions.value.filter(version => version.status === 'inactive').length
)
...
const filteredVersions = computed(() => {
  ...
  if (search.value) {
    ...
    filtered = filtered.filter(version => 
      version.version_name?.toLowerCase().includes(searchTerm)
    )
  }
