    ...
    <div class="mb-3 flex flex-col md:flex-row justify-between md:items-center gap-4">
      ...
      <div>
        ...
        <p class="text-gray-600">
          Add a new user role with permissions
        </p>
      </div>
...
const handleSubmit = async () => {
  try {
    ...
    showLoadingAlert('Creating role...')
    ...
    const response = await axios.post('/api/user-roles', payload)
    
    closeAlert()
    await showSuccessAlert('Success!', 'Role created successfully')
    
    router.push({ name: 'user-roles.index', query: { created: 'true' } })
    ...
    if (error.response?.data?.errors) {
      ...
      showErrorAlert('Error', error.response?.data?.message || 'Failed to create role')
    }
  }
}
